...
/datum/controller/subsystem/gamemode/proc/storyteller_vote_choices()
	...
	to_chat(world, vote_font(fieldset_block("Голосование за рассказчика", "[finalized_message]", "boxed_message purple_box")))
	return choices
...
/datum/controller/subsystem/gamemode/proc/init_storyteller()
	...
	if(!voted_storyteller)
		...
		log_dynamic("Раундстарт выбрал рассказчика [storyteller_pick.name] случайным образом из-за отсутствия результатов голосования.")
