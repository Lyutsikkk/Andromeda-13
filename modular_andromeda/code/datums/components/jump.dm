...
/datum/component/jump
	...
	var/jump_sound = "sound/weapons/thudswoosh.ogg"
	///Special jump behavior flags
	var/jump_flags
	///allow_pass_flags flags applied to the jumper on jump
	var/jumper_allow_pass_flags

/mob/living/proc/set_jump_component(duration = 0.5 SECONDS, cooldown = 1 SECONDS, cost = 10, height = 16, sound = null, flags = JUMP_SHADOW, flags_pass = PASSTABLE)
...
/datum/component/jump/Initialize(_jump_duration, _jump_cooldown, _stamina_cost, _jump_height, _jump_sound, _jump_flags, _jumper_allow_pass_flags)
	...
	RegisterSignal(parent, COMSIG_KB_MOB_JUMP, PROC_REF(do_jump))

/datum/component/jump/UnregisterFromParent()
	UnregisterSignal(parent, list(COMSIG_KB_MOB_JUMP, COMSIG_MOB_THROW))
...
///Actually sets the jump vars ///Фактически устанавливает переменные перехода
/datum/component/jump/proc/set_vars(_jump_duration = 0.5 SECONDS, _jump_cooldown = 1 SECONDS, _stamina_cost = 10, _jump_height = 16, _jump_sound = null, _jump_flags = JUMP_SHADOW, _jumper_allow_pass_flags = PASSTABLE)
...
///Performs the jump ///Выполняет прыжок
/datum/component/jump/proc/do_jump(mob/living/jumper)
	...
	if(stamina_cost && jumper.getStaminaLoss() < -stamina_cost)
		to_chat(jumper, span_warning("Переведи дыхание!"))
		return
	...
	if(jump_flags & JUMP_SPIN)
		...
		jumper.SpinAnimation(jump_duration / spin_number, spin_number, jumper.dir == WEST ? FALSE : TRUE)
...
///Ends the jump ///Завершает переход
...
///Jump throw bonuses ///Бонусы за бросок в прыжке
