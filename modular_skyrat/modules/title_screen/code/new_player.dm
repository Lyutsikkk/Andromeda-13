...
/mob/dead/new_player/proc/playerpolls()
	...
	if(query_get_new_polls.NextRow())
		output +={"<a class="menu_button menu_newpoll" href='byond://?src=[text_ref(src)];polls_menu=1'>ОПРОСЫ (Новое)</a>"}
	else
		output +={"<a class="menu_button" href='byond://?src=[text_ref(src)];polls_menu=1'>ОПРОСЫ</a>"}
	qdel(query_get_new_polls)
	if(QDELETED(src))
		return
	return output
