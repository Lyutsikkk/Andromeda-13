	...
	if(SSticker.current_state == GAME_STATE_STARTUP)
		...
		if(!SSticker || SSticker.current_state <= GAME_STATE_PREGAME)
			...
			dat += {"
				...
				<a class="menu_button" href='byond://?src=[text_ref(src)];view_manifest=1'>СПИСОК ЧЛЕНОВ ЭКИПАЖА</a>
			"}
			dat += {"<a class="menu_button" href='byond://?src=[text_ref(src)];character_directory=1'>СПРАВОЧНИК ПЕРСОНАЖА</a>"}

		dat += {"<a class="menu_button" href='byond://?src=[text_ref(src)];observe=1'>НАБЛЮДАТЬ</a>"}

		dat += {"
			<hr>
			<a class="menu_button" href='byond://?src=[text_ref(src)];character_setup=1'>НАСТРОЙКА ПЕРСОНАЖА (<span id="character_slot">[uppertext(client.prefs.read_preference(/datum/preference/name/real_name))]</span>)</a>
			<a class="menu_button" href='byond://?src=[text_ref(src)];game_options=1'>ИГРОВЫЕ НАСТРОЙКИ</a>
			...
			<!-- <hr> -->
			<!--  <a class="menu_button" href='byond://?src=[text_ref(src)];server_swap=1'>ПОМЕНЯТЬ СЕРВЕР</a> -->
		"}

		if(length(GLOB.lobby_station_traits))
			dat += {"<a class="menu_button" href='byond://?src=[text_ref(src)];job_traits=1'>ПРОФЕССИОНАЛЬНЫЕ НАВЫКИ</a>"}
