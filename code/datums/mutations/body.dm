...
/datum/mutation/dwarfism/on_acquiring(mob/living/carbon/human/owner)
	...
	if(owner.dna.features["body_size"] < 1 || isteshari(owner))
		to_chat(owner, "Вы чувствуете, как ваше тело пытается сжаться, но не органы! Ой-ой!")
...
/datum/mutation/dwarfism/on_losing(mob/living/carbon/human/owner)
	...
	// SKYRAT EDIT BEGIN
	if(owner.dna.features["body_size"] < 1 || isteshari(owner))
		to_chat(owner, "Вы чувствуете облегчение, поскольку ваши органы перестают напрягаться изнутри.")
...
/datum/mutation/gigantism/on_acquiring(mob/living/carbon/human/owner)
	...
	if(owner.dna.features["body_size"] > 1)
		to_chat(owner, "Вы чувствуете, как ваше тело расширяется еще больше, но при этом создается ощущение, что ваши кости расширяются слишком сильно!")
...
/datum/mutation/gigantism/on_losing(mob/living/carbon/human/owner)
	...
	// SKYRAT EDIT BEGIN
	if(owner.dna.features["body_size"] > 1)
		to_chat(owner, "Вы чувствуете облегчение, поскольку ваши кости прекращают бурный рост.")
		return
	// SKYRAT EDIT END
