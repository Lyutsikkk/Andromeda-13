/datum/brain_trauma/mild/phobia
	...
	scan_desc = "фобия"
	gain_text = span_warning("Вы начинаете находить значения по умолчанию очень нервными...")
	lose_text = span_notice("Вы больше не боитесь значений по умолчанию.")
	var/phobia_type
...
/datum/brain_trauma/mild/phobia/New(new_phobia_type)
	...
	lose_text = span_notice("Вы больше не испытываете страха перед [phobia_type].")
	scan_desc += " из [phobia_type]"
...
/datum/brain_trauma/mild/phobia/handle_speech(datum/source, list/speech_args)
	...
	to_chat(owner, span_warning("Вам трудно произнести слово \"[span_phobia("[trigger_regex.group[2]]")]\"!"))

/datum/brain_trauma/mild/phobia/proc/freak_out(atom/reason, trigger_word)
	if(owner.stat == DEAD)
		return

	var/message = pick("пугает до костей", "встряхивает", "приводит в ужас", "ввергает в панику", "мурашки по позвоночнику")
	if(trigger_word)
		if (owner.has_status_effect(/datum/status_effect/minor_phobia_reaction))
			return
		to_chat(owner, span_userdanger("Слышите [span_phobia(trigger_word)] [message]!"))
	...
	if(reason)
		to_chat(owner, span_userdanger("Видете [span_phobia(reason.name)] [message]!"))
	else
		to_chat(owner, span_userdanger("Нечто [message]!"))
