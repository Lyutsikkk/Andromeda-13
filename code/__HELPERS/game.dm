...
/proc/announce_arrival(mob/living/carbon/human/character, rank, announce_to_ghosts = TRUE)
	...
	if (announce_to_ghosts)
		...
		deadchat_broadcast(span_game(" прибыл на станцию за [span_name(player_area.name)]."), span_game("[span_name(character.real_name)] ([rank])"), follow_target = character, message_type=DEADCHAT_ARRIVALRATTLE)
 ...
 */
/proc/send_tip_of_the_round(target, selected_tip, source = "Совет для раунда")
	var/message
	if(selected_tip)
