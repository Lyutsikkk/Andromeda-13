 ...
 */
/proc/power_failure()
	priority_announce("Обнаружена аномальная активность в сети [station_name()]. В качестве меры предосторожности питание станции будет отключено на неопределенный срок.", "Критический сбой питания", ANNOUNCER_POWEROFF)
 ...
 */
/proc/power_restore()
	priority_announce("На [station_name()] восстановлено электропитание. Мы приносим извинения за причиненные неудобства.", "Системы питания в работе", ANNOUNCER_POWERON)
	for(var/obj/machinery/power/apc/C as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/apc))
		if(C.cell && is_station_level(C.z))
...
/proc/power_restore_quick()
	priority_announce("Все SMES на [station_name()] были перезаряжены. Мы приносим извинения за причиненные неудобства.", "Системы питания в работе", ANNOUNCER_POWERON)
	var/list/all_smes = SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/smes)
	for(var/obj/machinery/power/smes/smes as anything in all_smes)
		if(!is_station_level(smes.z))
			continue
