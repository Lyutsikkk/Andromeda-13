...
/obj/structure/falsewall/tool_act(mob/living/user, obj/item/tool, list/modifiers)
	if(!opening || !tool.tool_behaviour)
		return ..()
	to_chat(user, span_warning("Вы должны подождать, пока дверь перестанет двигаться!"))
	return ITEM_INTERACT_BLOCKING

/obj/structure/falsewall/screwdriver_act(mob/living/user, obj/item/tool)
	if(!density)
		to_chat(user, span_warning("Если вы не можете дотянуться, сначала закройте его!"))
		return
	var/turf/loc_turf = get_turf(src)
	if(loc_turf.density)
		to_chat(user, span_warning("[src] заблокирован!"))
		return ITEM_INTERACT_SUCCESS
	if(!isfloorturf(loc_turf))
		to_chat(user, span_warning("[src] болты должны быть закреплены на полу!"))
		return ITEM_INTERACT_SUCCESS
	user.visible_message(span_notice("[user] закручивает несколько болтов на стене."), span_notice("Вы затягиваете болты на стене."))
	ChangeToWall()
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/falsewall/proc/dismantle(mob/user, disassembled=TRUE, obj/item/tool = null)
	user.visible_message(span_notice("[user] разбираете фальшивую стену."), span_notice("Вы разбираете фальшивую стену."))
	if(tool)
...
/obj/structure/falsewall/examine_status(mob/user) //So you can't detect falsewalls by examine.
	to_chat(user, span_notice("Наружная обшивка прочно <b>приварена</b> к месту."))
	return null
...
/obj/structure/falsewall/reinforced/examine_status(mob/user)
	to_chat(user, span_notice("Внешняя <b>решетка</b> полностью сохранена."))
	return null
...
/obj/structure/falsewall/gold
	name = "золотая стена"
	desc = "Стена с золотым напылением. Swag!"
...
/obj/structure/falsewall/silver
	...
	desc = "Стена с серебряным покрытием. Блестящая."
...
/obj/structure/falsewall/diamond
	name = "алмазная стена"
	desc = "Стена с алмазным покрытием. Ты чудовище."
...
/obj/structure/falsewall/titanium
	name = "стена"
	desc = "Легкая титановая стенка, используемая в шаттлах."
...
/obj/structure/falsewall/material/finalize_material_effects(list/materials)
	. = ..()
	desc = "Огромная часть  [get_material_english_list(materials)] использовалась для разделения комнат."
