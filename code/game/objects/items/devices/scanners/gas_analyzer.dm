/obj/item/analyzer
	name = "газан"
	desc = "Ручной сканер состояния окружающей среды, который сообщает о текущем уровне загазованности. Made in Srakov."
...
/obj/item/analyzer/examine(mob/user)
	. = ..()
	. += span_notice("Нажмите ПКМ [src], чтобы открыть справочник по газу.")
	. += span_notice("Нажмите ЛКМ [src], чтобы активировать функцию барометра.")

/obj/item/analyzer/suicide_act(mob/living/carbon/user)
	user.visible_message(span_suicide("[user] начинает анализировать [user.p_them()] себя с помощью [src]! На дисплее отображается, что [user.p_theyre()] мертв!"))
	return BRUTELOSS

/obj/item/analyzer/click_alt(mob/user) //Barometer output for measuring when the next storm happens
	if(cooldown)
		to_chat(user, span_warning("Функция барометра [src] готовится к запуску."))
	...
	if(!user_area.outdoors)
		to_chat(user, span_warning("Функция барометра [src] не будет работать в помещении!"))
		return CLICK_ACTION_BLOCKING
	...
	if(ongoing_weather)
		if((ongoing_weather.stage == MAIN_STAGE) || (ongoing_weather.stage == WIND_DOWN_STAGE))
			to_chat(user, span_warning("Функция барометра [src] не может ничего отследить, пока шторм продолжается [ongoing_weather.stage == MAIN_STAGE ? "уже здесь!" : "стихает."]"))
			return CLICK_ACTION_BLOCKING

		to_chat(user, span_notice("Следующая [ongoing_weather] наступит в [butchertime(ongoing_weather.next_hit_time - world.time)]."))
		if(ongoing_weather.aesthetic)
			to_chat(user, span_warning("Функция барометра [src] говорит о том, что следующий шторм пронесется мимо."))
	else
		var/next_hit = SSweather.next_hit_by_zlevel["[T.z]"]
		var/fixed = next_hit ? timeleft(next_hit) : -1
		if(fixed < 0)
			to_chat(user, span_warning("Функция барометра [src] не смогла отследить какие-либо погодные условия."))
		else
			to_chat(user, span_warning("Функция барометра [src] показывает, что шторм начнется примерно через [butchertime(fixed)]."))
	cooldown = TRUE
...
/obj/item/analyzer/proc/ping()
	if(isliving(loc))
		...
		to_chat(L, span_notice("Функция барометра [src] готова!"))
...
/proc/atmos_scan(mob/user, atom/target, silent=FALSE)
	...
	for(var/datum/gas_mixture/air as anything in airs)
		...
		if(total_moles > 0)
			message += span_notice("Моль: [round(total_moles, 0.01)]")
			...
			message += span_notice("Температура: [round(temperature - T0C,0.01)] &deg;Градусов ([round(temperature, 0.01)] Кельвин)")
			message += span_notice("Объем: [volume] Литров")
			message += span_notice("Давление: [round(pressure, 0.01)] кПа")
			message += span_notice("Теплоемкость: [display_energy(heat_capacity)] / Кельвин")
			message += span_notice("Тепловая энергия: [display_energy(thermal_energy)]")
		else
			message += airs.len > 1 ? span_notice("Этот узел пуст!") : span_notice("[target] пуста!")
			message += span_notice("Объем: [volume] Литров") // don't want to change the order volume appears in, suck it
...
/obj/item/analyzer/ranged
	desc = "Ручной сканер окружающей среды дальнего действия, который сообщает о текущем уровне загазованности. Made in Srakov."
	name = "газан дальнего действия"
