...
/proc/healthscan(mob/user, mob/living/target, mode = SCANNER_VERBOSE, advanced = FALSE, tochat = TRUE)
	...
	if(ishuman(target))
		...
		render_list += "<span class='info ml-1'>Раса: <b>[species_name]</b></span><br>" // Bubber Edit: Bold species name
	...
	if(blood_id)
		...
		if(carbontarget.blood_volume <= BLOOD_VOLUME_SAFE && carbontarget.blood_volume > BLOOD_VOLUME_OKAY)
			level_format = conditional_tooltip("НИЗКИЙ [blood_percent]%, [carbontarget.blood_volume] cl", "Рекомендации: [blood_type.restoration_chem::name] добавка.", tochat)
		else if(carbontarget.blood_volume <= BLOOD_VOLUME_OKAY)
			level_format = conditional_tooltip("<b>КРИТИЧЕСКИЙ [blood_percent]%</b>, [carbontarget.blood_volume] cl", "Рекомендации: [blood_type.restoration_chem::name] добавка, [/datum/reagent/medicine/salglu_solution::name] или переливание крови.", tochat)
		else
			level_format = "[blood_percent]%, [carbontarget.blood_volume] cl"

		blood_type_format = "группа: [blood_type]"
		...
		render_list += "<span class='[carbontarget.blood_volume < BLOOD_VOLUME_SAFE ? "alert" : "info"] ml-1'>Уровень крови: [level_format],</span> <span class='info'>[blood_type_format]</span><br>"
