...
/proc/healthscan(mob/user, mob/living/target, mode = SCANNER_VERBOSE, advanced = FALSE, tochat = TRUE)
	...
	if(blood_type)
		...
		if(target.blood_volume <= BLOOD_VOLUME_SAFE && target.blood_volume > BLOOD_VOLUME_OKAY)
			level_format = "НИЗКИЙ [blood_percent]%, [target.blood_volume] cl"
			...
			level_format = "<b>КРИТИЧЕСКИЙ [blood_percent]%</b>, [target.blood_volume] cl"
		...
		render_list += "<span class='[target.blood_volume < BLOOD_VOLUME_SAFE ? "alert" : "info"] ml-1'>[blood_type.get_blood_name()] Уровень крови: [level_format],</span> <span class='info'>[blood_type_format]</span><br>"
