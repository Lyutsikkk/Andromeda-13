...
/datum/action/innate/ai/nuke_station/proc/set_up_us_the_bomb(mob/living/owner)
	...
	if (owner_AI.stat != DEAD)
		priority_announce("Во всех системах станции обнаружены враждебные объекты, пожалуйста, деактивируйте свой ИИ, чтобы предотвратить возможное повреждение его ядра.", "Предупреждение об аномалии", ANNOUNCER_AIMALF)
