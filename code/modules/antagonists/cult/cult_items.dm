...
/obj/item/shuttle_curse/attack_self(mob/living/user)
	...
	if(SSshuttle.emergency.mode == SHUTTLE_CALL)
		...
		if(MAX_SHUTTLE_CURSES-totalcurses <= 0)
			to_chat(user, span_danger(span_big("Вы чувствуете, что аварийный шаттл больше не может быть проклят. Было бы неразумно создавать новые проклятые орбы.")))
		else if(MAX_SHUTTLE_CURSES-totalcurses == 1)
			to_chat(user, span_danger(span_big("Вы чувствуете, что аварийный шаттл можно проклясть только еще раз.")))
		else
			to_chat(user, span_danger(span_big("Вы чувствуете, что аварийный шаттл может быть проклят большее [MAX_SHUTTLE_CURSES-totalcurses] количество раз.")))
		...
		if(totalcurses >= MAX_SHUTTLE_CURSES && (world.time < first_curse_time + SHUTTLE_CURSE_OMFG_TIMESPAN))
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(priority_announce), omfg_message, "Приоритетное оповещение", 'sound/announcer/announcement/announce_syndi.ogg', null, "Нанотразен Департамент транспорта: Центральное Командование"), rand(2 SECONDS, 6 SECONDS))
