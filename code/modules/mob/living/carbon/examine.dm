...
/mob/living/carbon/examine(mob/user)
	...
	if(temporary_flavor_text)
		if(length_char(temporary_flavor_text) < TEMPORARY_FLAVOR_PREVIEW_LIMIT)
			. += span_revennotice("<br>Выглядят иначе, чем обычно: [temporary_flavor_text]")
		else
			. += span_revennotice("<br>Выглядят иначе, чем обычно: [copytext_char(temporary_flavor_text, 1, TEMPORARY_FLAVOR_PREVIEW_LIMIT)]... <a href='byond://?src=[REF(src)];temporary_flavor=1'>More...</a>")

	if(client)
		var/erp_status_pref = client.prefs.read_preference(/datum/preference/choiced/erp_status)
		if(erp_status_pref && !CONFIG_GET(flag/disable_erp_preferences))
			. += EXAMINE_SECTION_BREAK
			. += span_info("ЕРП Статус: [span_revenboldnotice(erp_status_pref)]")
	// SKYRAT EDIT END
...
/mob/living/carbon/proc/get_clothing_examine_info(mob/living/user)
	...
	for(var/obj/item/bodypart/arm/part in bodyparts)
		...
		. += "[t_He] [corresponding_item.examine_title(user)] вместо [initial(part.plaintext_zone)]." // BUBBER EDIT CHANGE - ORIGINAL: . += "[t_He] [t_has] a [corresponding_item.examine_title(user)] in place of [t_his] [initial(part.plaintext_zone)]."
	//gloves
...
/mob/living/carbon/human/get_clothing_examine_info(mob/living/user)
	...
	if(w_shirt && !undershirt_hidden() && !HAS_TRAIT(w_shirt, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [w_shirt.examine_title(user, declent = ACCUSATIVE)]."
	// SPLURT EDIT - bra
	if(w_bra && !bra_hidden() && !HAS_TRAIT(w_bra, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [w_bra.examine_title(user, declent = ACCUSATIVE)]."
	// SPLURT EDIT - underwear
	if(w_underwear && !underwear_hidden() && !HAS_TRAIT(w_underwear, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [w_underwear.examine_title(user, declent = ACCUSATIVE)]."
	//head
	if(head && !(obscured & ITEM_SLOT_HEAD) && !HAS_TRAIT(head, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [head.examine_title(user, declent = ACCUSATIVE)] на голове."
	// SPLURT EDIT - socks
	if(w_socks && !socks_hidden() && !HAS_TRAIT(w_socks, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [w_socks.examine_title(user, declent = ACCUSATIVE)] на ногах."
	//mask
	if(wear_mask && !(obscured & ITEM_SLOT_MASK)  && !HAS_TRAIT(wear_mask, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [wear_mask.examine_title(user, declent = ACCUSATIVE)] на лице."
	//neck
	...
	if(!(obscured & ITEM_SLOT_EYES) )
		if(glasses  && !HAS_TRAIT(glasses, TRAIT_EXAMINE_SKIP))
			...
			. += span_warning("<B>[t_His] глаза налиты кровью!</B>")
	//ears
	if(ears && !(obscured & ITEM_SLOT_EARS_LEFT) && !HAS_TRAIT(ears, TRAIT_EXAMINE_SKIP)) // SPLURT EDIT - ears extra
		. += "[t_He] носит [ears.examine_title(user, declent = ACCUSATIVE)] на левом ухе." // extra inventory

	// SPLURT EDIT - ears extra
	if(ears_extra && !(obscured & ITEM_SLOT_EARS_RIGHT) && !HAS_TRAIT(ears_extra, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [ears_extra.examine_title(user, declent = ACCUSATIVE)] на правом ухе."

	// SPLURT EDIT - ears extra
	//wearing two ear items makes you look like an idiot
	if((istype(ears, /obj/item/radio/headset) && !(obscured & ITEM_SLOT_EARS_LEFT) && !HAS_TRAIT(ears, TRAIT_EXAMINE_SKIP)) && (istype(ears_extra, /obj/item/radio/headset) && !(obscured & ITEM_SLOT_EARS_RIGHT) && !HAS_TRAIT(ears_extra, TRAIT_EXAMINE_SKIP)))
		. += span_warning("[t_He] выглядит довольно безвкусно, когда носит [ears.name] и [ears_extra.name] на голове.")
	...
	for(var/obj/item/bodypart/arm/part in bodyparts)
		if(!(part.bodypart_flags & BODYPART_PSEUDOPART))
			continue
		var/obj/item/corresponding_item = get_item_for_held_index(part.held_index) || part
		. += "[t_He] [corresponding_item.examine_title(user)] вместо [initial(part.plaintext_zone)]." // BUBBER EDIT CHANGE - ORIGINAL: . += "[t_He] [t_has] [corresponding_item.examine_title(user)] in place of [t_his] [initial(part.plaintext_zone)]."
	//gloves
	if(gloves && !(obscured & ITEM_SLOT_GLOVES) && !HAS_TRAIT(gloves, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [gloves.examine_title(user, declent = ACCUSATIVE)] на руках."
	...
	if(wrists && !wrists_hidden() && !HAS_TRAIT(wrists, TRAIT_EXAMINE_SKIP))
		. += "[t_He] носит [wrists.examine_title(user, declent = ACCUSATIVE)] на запястье."
	//handcuffed?
	if(handcuffed)
		var/cables_or_cuffs = istype(handcuffed, /obj/item/restraints/handcuffs/cable) ? "в связках" : "в наручниках"
		. += span_warning("[t_He] [icon2html(handcuffed, user)] [cables_or_cuffs]!")
