...
/mob/living/proc/MobBump(mob/M)
	...
	if(isliving(M))
		...
		if(L.gunpointed.len)
			...
			if(!is_pointing)
				if(!(world.time % 5))
					to_chat(src, "<span class='warning'>[capitalize(L.declent_ru(NOMINATIVE))] держат под прицелом, давить на него неразумно.</span>")
				return TRUE
		if(L.gunpointing)
			if(!(world.time % 5))
				to_chat(src, "<span class='warning'>[capitalize(L.declent_ru(NOMINATIVE))] держит кого-то под дулом пистолета, мимо него нельзя протиснуться.</span>")
			return TRUE
		//SKYRAT EDIT ADDITION END
		...
		if(!supress_message && !(iscarbon(AM) && HAS_TRAIT(src, TRAIT_STRONG_GRABBER)))
			...
			if(ishuman(M))
				if(zone_selected == BODY_ZONE_PRECISE_GROIN && M.get_organ_slot(ORGAN_SLOT_EXTERNAL_TAIL) && src.get_organ_slot(ORGAN_SLOT_EXTERNAL_TAIL))
					M.visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] извивает хвост с помощью [AM], вау, это нормально на публике?!"), "[capitalize(declent_ru(NOMINATIVE))] переплел свой хвост с вашим!")
					to_chat(src, "Ты обвиваешь свой хвост [AM]")
				else
...
/mob/living/resist_grab(moving_resist)
	...
	if(effective_grab_state >= GRAB_AGGRESSIVE)
		...
		if(prob(resist_chance))
			...
			if(HAS_TRAIT(src, TRAIT_SLIPPERY))
				visible_message(span_cyan("[capitalize(declent_ru(NOMINATIVE))] выскальзывает из рук [pulledby.declent_ru(GENITIVE)]!"), \
								span_cyan("Ты выскользнул из рук [pulledby.declent_ru(GENITIVE)]!"), null, null, pulledby)
				to_chat(pulledby, span_cyan("[capitalize(declent_ru(NOMINATIVE))] выскальзывает из рук!"))
