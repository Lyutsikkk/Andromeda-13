...
/mob/living/proc/process_capture(ransom_price, black_market_price)
	if(ransom_price > 0)
		...
		priority_announce("Один из членов вашей команды был захвачен конкурирующей организацией, и нам пришлось заплатить за него выкуп, чтобы вернуть его. В соответствии с правилами мы взяли часть средств станции, чтобы компенсировать общие расходы.", "Нанотразен Защита Активов", has_important_message = TRUE)
