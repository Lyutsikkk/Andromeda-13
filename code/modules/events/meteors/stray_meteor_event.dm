...
/datum/round_event/stray_meteor/announce(fake)
	if(length(GLOB.meteor_list))
		...
		priority_announce("Наши сенсоры [sensor_name] обнаружили входящую сигнатуру, приближающуюся к [GLOB.station_name]. Пожалуйста, приготовьтесь к столкновению.", "Предупреждение о метеоритах")
