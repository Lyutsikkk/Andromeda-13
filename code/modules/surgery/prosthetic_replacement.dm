...
/datum/surgery_step/add_prosthetic/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(istype(tool, /obj/item/borg/apparatus/organ_storage))
		...
		if(!isbodypart(organ_storage_contents))
			to_chat(user, span_warning("Нельзя прикрепить [organ_storage_contents]!"))
			return SURGERY_STEP_FAIL
	...
	if(isbodypart(tool))
		...
		if(target_zone == bodypart_to_attach.body_zone) //so we can't replace a leg with an arm, or a human arm with a monkey arm.
			display_results(
				user,
				target,
				span_notice("Вы начинаете заменять [target.parse_zone_with_bodypart(target_zone)] у [target] на [tool]..."),
				span_notice("[user] начинает заменять [target.parse_zone_with_bodypart(target_zone)] у [target] на [tool]."),
		...
		display_results(
			...
			span_notice("[user] начинает прикреплять [tool] к [target.parse_zone_with_bodypart(target_zone)] у [target]."),
			span_notice("[user] начинает прикреплять что-то к [target.parse_zone_with_bodypart(target_zone)] у [target]."),
		)
	else
		to_chat(user, span_warning("[tool] устанавливается только в руку."))
		return SURGERY_STEP_FAIL
