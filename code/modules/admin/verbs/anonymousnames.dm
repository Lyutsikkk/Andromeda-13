...
/datum/anonymous_theme/proc/announce_to_all_players()
	priority_announce("Недавняя бюрократическая ошибка привела к необходимости полного отзыва данных всех личностей до дальнейшего уведомления.", "Потеря личности", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/proc/restore_all_players()
	priority_announce("Имена и личности были восстановлены.", "Восстановление личности", SSstation.announcer.get_rand_alert_sound())
	for(var/mob/living/player in GLOB.player_list)
		if(!player.mind || (!ishuman(player) && !issilicon(player)) || player.mind.assigned_role.faction != FACTION_STATION)
			continue
...
/datum/anonymous_theme/employees/announce_to_all_players()
	priority_announce("В качестве наказания за низкую производительность этой станции по сравнению с соседними станциями, личности будут ограничены до дальнейшего уведомления.", "Финансовый отчет", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/wizards/announce_to_all_players()
	priority_announce("Ваша станция попала в поле зрения Меметической Опасности Федерации Волшебников. Вы не являетесь собой, и вы% a2E 34!NOT4--- Добро пожаловать в Академию, ученики!", "Меметическая опасность", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/spider_clan/announce_to_all_players()
	priority_announce("Ваша станция была продана клану Пауков. Ваши новые обозначения будут применены сейчас.", "Новое управление", SSstation.announcer.get_rand_alert_sound())

/datum/anonymous_theme/spider_clan/anonymous_ai_name(is_ai = FALSE)
	var/posibrain_name = pick(GLOB.posibrain_names)
	if(is_ai)
		return "Храмовник Шаолиня [posibrain_name]"
	else
		var/martial_prefix = capitalize(pick(GLOB.martial_prefix))
		var/martial_style = pick("Обезьяна", "Тигр", "Гадюка", "Богомол", "Журавль", "Панда", "Летучая мышь", "Медведь", "Сороконожка", "Лягушка")
...
/datum/anonymous_theme/station/announce_to_all_players()
	priority_announce("Подтверждено событие ошибки реальности 9-го уровня в районе [station_name()]. Весь персонал должен стараться изо всех сил продолжать работу, чтобы случайно не спровоцировать новые события реальности.", "[command_name()] Высшие Измерение", 'sound/announcer/notice/notice1.ogg')
