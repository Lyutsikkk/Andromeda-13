...
/datum/sm_delam/proc/delam_progress(obj/machinery/power/supermatter_crystal/sm)
	...
	if (sm.damage >= sm.emergency_point && sm.damage_archived < sm.emergency_point)
		sm.investigate_log("вошел в критическую точку.", INVESTIGATE_ENGINE)
		message_admins("[sm] вошел в критическую точку [ADMIN_VERBOSEJMP(sm)].")
	...
	*/
	...
	if(sm.damage >= sm.emergency_point) // In emergency
		sm.radio.talk_into(sm, "НЕИЗБЕЖНОЕ РАССЛОЕНИЕ КРИСТАЛЛА! Целостность: [round(sm.get_integrity_percent(), 0.01)]%", sm.emergency_channel)
		...
		sm.radio.talk_into(sm, "Опасность! Нарушена целостность кристаллической гиперструктуры! Целостность: [round(sm.get_integrity_percent(), 0.01)]%", sm.warning_channel)
