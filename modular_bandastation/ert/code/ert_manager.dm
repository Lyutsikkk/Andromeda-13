	...
	switch(action)
		...
		if("dispatchErt")
			...
			switch(ert_type)
				...
				if("Epsilon")
					new_ert = new /datum/ert/deathsquad
				else
			...
			if((commander_slots + medical_slots + janitor_slots + inquisitor_slots + security_slots + engineering_slots) == 0)
				message_admins("[key_name_admin(usr)] попытался создать [ert_type] ОБР с нулевым количеством доступных слотов!")
				log_admin("[key_name(usr)] попытался создать [ert_type] ОБР с нулевым количеством доступных слотов.")
				to_chat(usr, span_userdanger("В ОБР должен быть доступен как минимум 1 слот!"))
				return
			...
			message_admins("[key_name_admin(usr)] начал набор [ert_type] ОБР. Слоты для набора: [slot_text]")
			log_admin("[key_name(usr)] начал набор [ert_type] ОБР. Слоты для набора: [slot_text]")
			if(should_be_announced)
				priority_announce("Внимание, [station_name()]. Мы рассматриваем возможность отправки ОБР, ожидайте.", "Активирован протокол ОБР", sound = 'sound/announcer/default/obr_request.ogg')
		...
		if("denyErt")
			...
			priority_announce(message, "ОБР недоступно", sound = 'sound/announcer/default/obr_no.ogg')
		else
			return FALSE
...
/datum/ert_manager/proc/makeERTFromSlots(datum/ert/ertemplate, admin_slots, commander_slots, security_slots, medical_slots, engineering_slots, janitor_slots, inquisitor_slots)
	...
	var/list/mob/dead/observer/candidates = SSpolling.poll_ghost_candidates("Хотите ли вы, чтобы вас рассматривали для участия в [span_notice(ertemplate.polldesc)]?", check_jobban = "deathsquad", alert_pic = /obj/item/card/id/advanced/centcom/ert, role_name_text = "Отряд Быстрого Реагирования")
	...
	if(!length(candidates))
		if(should_be_announced)
			priority_announce("Внимание, [station_name()]. К сожалению, в настоящее время мы не можем направить к вам отряд быстрого реагирования.", "ОБР недоступен", sound = 'sound/announcer/default/obr_no.ogg')
		return FALSE
	...
	if(ertemplate.spawn_admin)
		if(isobserver(usr))
			...
			to_chat(usr, span_warning("Ты не можешь стать инструктором, т.к. не призрак."))
	...
	while(numagents && candidates.len)
		...
		//Logging and cleanup
		ert_operative.log_message("был выбран как [ert_antag.name].", LOG_GAME)
		numagents--
		teamSpawned++

	if (teamSpawned)
		message_admins("[ertemplate.polldesc] появился с миссией [ertemplate.mission]")
		if(should_be_announced)
			switch(ert_type)
				if("Red")
					priority_announce("Внимание, [station_name()]. Мы направляем отряд быстрого реагирования кода «КРАСНЫЙ». Ожидайте.", "ОБР в пути", sound = 'sound/announcer/default/obr_yes.ogg')
				if("Gamma")
					priority_announce("Внимание, [station_name()]. Мы направляем элитный отряд быстрого реагирования кода «ГАММА». Ожидайте.", "ОБР в пути", sound = 'sound/announcer/default/obr_yes.ogg')
				if("Epsilon")
					priority_announce("Нарушение контракта с корпорацией Нанотрейзен. Специальное подразделение направлено на станцию, чтоб забрать активы корпорации. Сохраняйте спокойствие, спасибо что выбрали корпорацию Нанотрейзен!", "Департамент Специальных Операций", sound = 'sound/announcer/default/deathsquad.ogg')
