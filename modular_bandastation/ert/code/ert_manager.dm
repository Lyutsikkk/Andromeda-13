	...
	switch(action)
		...
		if("dispatchErt")
			...
			if(should_be_announced)
				priority_announce("Внимание, [station_name()]. Мы рассматриваем возможность отправки ОБР, ожидайте.", "Активирован протокол ОБР", sound = 'sound/announcer/classic/obr_request.ogg')
		...
		if("denyErt")
			...
			priority_announce(message, "ОБР недоступно", sound = 'sound/announcer/classic/obr_no.ogg')
		else
			return FALSE
...
/datum/ert_manager/proc/makeERTFromSlots(datum/ert/ertemplate, admin_slots, commander_slots, security_slots, medical_slots, engineering_slots, janitor_slots, inquisitor_slots)
	...
	if(!length(candidates))
		if(should_be_announced)
			priority_announce("Внимание, [station_name()]. К сожалению, в настоящее время мы не можем направить к вам отряд быстрого реагирования.", "ОБР недоступен", sound = 'sound/announcer/classic/obr_no.ogg')
		return FALSE
	...
	if (teamSpawned)
		...
		if(should_be_announced)
			switch(ert_type)
				if("Red")
					priority_announce("Внимание, [station_name()]. Мы направляем отряд быстрого реагирования кода «КРАСНЫЙ». Ожидайте.", "ОБР в пути", sound = 'sound/announcer/classic/obr_yes.ogg')
				if("Gamma")
					priority_announce("Внимание, [station_name()]. Мы направляем элитный отряд быстрого реагирования кода «ГАММА». Ожидайте.", "ОБР в пути", sound = 'sound/announcer/classic/obr_yes.ogg')
				if("Epsilon")
					priority_announce("Нарушение контракта с корпорацией Нанотрейзен. Специальное подразделение направлено на станцию, чтоб забрать активы корпорации. Сохраняйте спокойствие, спасибо что выбрали корпорацию Нанотрейзен!", "Департамент Специальных Операций", sound = 'sound/announcer/classic/deathsquad.ogg')
