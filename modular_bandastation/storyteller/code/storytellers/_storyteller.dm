...
/datum/storyteller
	...
	var/roundstart_cap_multiplier = 1
...
/datum/storyteller/proc/find_and_buy_event_from_track(track)
	...
	if(length(SSgamemode.forced_next_events[track]))
		picked_event = pick(SSgamemode.forced_next_events[track])
		SSgamemode.forced_next_events[track] -= picked_event
		are_forced = TRUE
	else
		...
		if(!length(valid_events))
			//message_admins("Рассказчику не удалось выбрать событие для отслеживания [track].")
			mode.event_track_points[track] *= TRACK_FAIL_POINT_PENALTY_MULTIPLIER
			return
	...
	if(picked_event?.can_spawn_event(get_active_player_count(alive_check = TRUE, afk_check = TRUE, human_check = TRUE)) && track && !are_forced)
